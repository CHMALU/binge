...
export type AuthDict = {
  ...
  confirmPassword: string; name: string; loginButton: string; registerButton: string;
  orContinueWith: string; google: string; noAccount: string; haveAccount: string;
  loginLink: string; registerLink: string; tagline: string; loginSuccess: string;
  registerSuccess: string; errorInvalidCredentials: string; errorEmailInUse: string;
  errorGeneric: string; errorPasswordMismatch: string; errorPasswordShort: string;
};

type Props = { lang: string; t: AuthDict; modal?: boolean };

export default function LoginForm({ lang, t, modal = false }: Props) {
  ...
  return (
    <div className="bg-binge-card border border-binge-border rounded-2xl p-8">
      ...
      <p className="text-center mt-5 text-sm text-binge-muted">
        {t.noAccount}{" "}
        <Link href={`/${lang}/register`} replace={modal} className="text-binge-gold font-semibold hover:underline">
          {t.registerLink}
        </Link>
      </p>
    </div>
