...
function renderPage(lang: string = "en") {
  return WatchlistPage({ params: Promise.resolve({ lang }) }).then((ui) => render(ui));
}
...
describe("Watchlist page", () => {
  ...
  it("respects locale when redirecting", async () => {
    mockedAuth.mockResolvedValue(null);

    await expect(renderPage("pl")).rejects.toThrow("NEXT_REDIRECT:/pl/login");
  });
  ...
  it("renders the empty state when the watchlist is empty", async () => {
    ...
    expect(screen.getByText(enDict.watchlist.title)).toBeInTheDocument();
    expect(screen.getByText(enDict.watchlist.empty)).toBeInTheDocument();
    expect(mockedMovie).not.toHaveBeenCalled();
    expect(mockedTv).not.toHaveBeenCalled();
  });

  it("renders items fetched from TMDb in addedAt-desc order", async () => {
    mockedAuth.mockResolvedValue({ user: { id: "u-1" } });
    mockedFindMany.mockResolvedValue([
      {
        id: "w1",
        ...
        tmdbId: 27205,
        mediaType: "movie",
        addedAt: new Date("2026-05-20T10:00:00.000Z"),
      },
      {
        id: "w2",
        userId: "u-1",
        tmdbId: 1399,
        mediaType: "tv",
        addedAt: new Date("2026-05-10T08:00:00.000Z"),
      },
    ]);
